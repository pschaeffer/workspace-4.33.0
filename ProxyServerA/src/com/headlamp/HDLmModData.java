...
/**
 * HDLmModData short summary.
 *
 * HDLmModData description.
 *
 * @version 1.0
 * @author Peter
 */
	...
	protected HDLmModData(JsonElement jsonElement) {
	  ...
	  super();
		...
		/* Set the error count to zero. The error count is incremented each time an
			 ...
			 from ever being used). field is set false). Note that a reference is used 
	  ...
	  MutableInt   errorCounter = new MutableInt(0);
		...
		if (errorMessages == null) {
			String  errorText = "Error message ArrayList allocation in HDLmModData constructor is null";
			throw new NullPointerException(errorText);
		}
