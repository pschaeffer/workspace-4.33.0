...
/**
 * HDLmModReport short summary.
 *
 * HDLmModReport description.
 *
 * @version 1.0
 * @author Peter
 */
...
public class HDLmModReport extends HDLmMod {
  ...
  protected HDLmModReport() { 
	...
	protected HDLmModReport(final JsonElement jsonElement) {
  	...
  	if (errorMessages == null) {
	  	String  errorText = "Error message ArrayList allocation in HDLmModReport constructor is null";
		  throw new NullPointerException(errorText);
  	}
	  ...
	  curReportType = HDLmMod.modFieldReportType(editorType, 
	  		                                       errorCounter,
	  		                                       errorMessages,
	                                             jsonObject, jsonKeys, 
		                                           "reportType", 
	  ...
	  reportType = curReportType;
		/* Mark the current report definition object as not executable,
		   if the error count was greater than zero. This is actually
		   done by setting the use mode field to a special value. */
  ...
  Integer                     countLines;
  HDLmModLines                overallLines = null;
  HDLmModLines                validLines = null;
  HDLmModLines                invalidLines = null;
  HDLmModList                 ignoreList = null;
  ...
  protected void         addLine(final HDLmModLine reportLine) {
    ...
    HDLmModLines  overallLinesRef = overallLines;
  ...
  protected void         addLinesReport() {
	  ...
	  String  invalidLinesName =  HDLmDefines.getString("HDLMINVALIDNODENAME");
		...
		HDLmModLines   invalidLines = new HDLmModLines(invalidLinesName);
	  ...
	  String  overallLinesName =  HDLmDefines.getString("HDLMOVERALLNODENAME");
		...
		HDLmModLines   overallLines = new HDLmModLines(overallLinesName);
	  ...
	  String  validLinesName =  HDLmDefines.getString("HDLMVALIDNODENAME");
		...
		HDLmModLines   ValidLines = new HDLmModLines(validLinesName);
		if (ValidLines == null)
  ...
  /* This routine builds a new report (not a tree node) with a unique
	...
	protected static HDLmModReport  buildNewReport(final HDLmModCompany company,
  	...
  	if (reportType == null) {
		...
		/* Create an empty report instance. The lines of the report may 
		   be added later. */ 
		HDLmModReport  report = new HDLmModReport();
		if (report == null)
		...
		/* We now want to set the name of the report. The name is obtained
		   ...
		   name. */ 
		String  reportName = HDLmModReport.getNextReportName(company);
  ...
  /* This routine builds a new report (not a tree node) with a unique
	...
	protected static HDLmModReport  buildNewReportExtended(final HDLmModCompany company,
			                                                        final HDLmReportTypes reportType) {
		/* Check if the company reference passed by the caller is null */
		if (company == null) {
	 	  String  errorText = "Company reference passed to buildNewReport is null";
		...
		/* Create an empty report instance. The lines of the report are 
	     added below. */ 
		HDLmModReport  report = HDLmModReport.buildNewReport(company,
				                                                 reportType);
		if (report == null)
	...
	protected static String  buildReportName(final HDLmModCompany company,
			                                     int reportNumber) {
		/* Check if the company reference passed by the caller is null */
		if (company == null) {
	...
	protected static HDLmTree  buildTreeExtended(final ArrayList<String> oldNodePath, 
		...
		/* Build a new tree node with the correct information */
		HDLmTree  reportTree = HDLmModReport.buildTree(oldNodePath, 
                                                        newValue);
		if (reportTree == null) {
			String  errorText = "New report tree node was not created";
			HDLmAssertAction(false, errorText);
		...
		HDLmModLines   newInvalidLines = newValue.getInvalidLinesRef();
		if (newInvalidLines == null) {
			newInvalidLines = new HDLmModLines();
			if (newInvalidLines == null)
		...
		HDLmTree  invalidLinesTree = HDLmModLines.buildTreeExtended(nodePath, 
                                                                     newInvalidLines);
		...
		HDLmModLines   newOverallLines = newValue.getOverallLinesRef();
		if (newOverallLines == null) {
			newOverallLines = new HDLmModLines();
			if (newOverallLines == null)
		...
		HDLmTree  overallLinesTree = HDLmModLines.buildTreeExtended(nodePath, 
				                                                             newOverallLines);
		...
		HDLmModLines   newValidLines = newValue.getValidLinesRef();
		if (newValidLines == null) {
			newValidLines = new HDLmModLines();
			if (newValidLines == null)
       ...
       will have children for all of the actual valid lines. */
		HDLmTree  validLinesTree = HDLmModLines.buildTreeExtended(nodePath, 
				                                                           newValidLines);
		/* Build a tree node for the ignore-list associated with every report.
		   ...
		   will be bypassed. */  
	  HDLmModList   newIgnoreList = newValue.getIgnoreListRef();
	  ...
	  if (newIgnoreList != null)
	  	 ignoreListTree = HDLmModList.buildTreeExtended(nodePath, 
                                                           newIgnoreList);
  ...
  protected static boolean  checkReport(final HDLmModReport report,
		...
		/* Make a copy of the ignore-list object and store a reference
		   ...
		   will be saved. */ 
		HDLmModList  ignoreListCopy = new HDLmModList(ignoreList);
  ...
  protected static boolean  checkReportLines(final HDLmModReport report,
  	...
  	boolean   rv = true;
		/* Get the reference to the overall set of report lnes. This 
	     is a reference to a lines instance. */
	  HDLmModLines  overallLinesRef = report.overallLines;
	  if (overallLinesRef == null)
	  	HDLmAssertAction(false, "Reference to overall lines from report is null");
  	/* Check every report line */
  	for (Map.Entry<String, HDLmModLine> entry : overallLinesRef.getLines().entrySet()) {
  		String            reportLineName = entry.getKey();
  	  HDLmModLine  reportLine = entry.getValue();
  ...
  protected HDLmModList  getIgnoreListRef() {
  	return ignoreList;
  }
  /* Get the invalid lines reference and return it to the caller */
  protected HDLmModLines  getInvalidLinesRef() {
  	return invalidLines;
  }
  /* This routine returns the next report name. The next report name
	   ...
	   not a real problem. */  
	protected static String  getNextReportName(final HDLmModCompany company) {
		/* Check if the company reference passed by the caller is null */
		if (company == null) {
	  ...
	  /* If the current high report name could not be found, just return
		...
		if (trailingDigits.length() > 0) {
			...
			finalReportName = HDLmModReport.buildReportName(company,
					                                                 trailingInteger);
			return finalReportName;
		}
  ...
  protected HDLmModLines  getOverallLinesRef() {
  	return overallLines;
  }
  ...
  protected HDLmModLines  getValidLinesRef() {
  	return validLines;
  }
  ...
  protected final void   setIgnoreListRef(final HDLmModList newValue) {
    /* Check if the ignore-list reference passed by the caller is null */
  	if (newValue == null) {
	...
	protected final void   setInvalidRef(final HDLmModLines newValue) {
	...
	protected final void   setOverallRef(final HDLmModLines newValue) {
  ...
  protected final void   setMatchCacheNull() {
	...
	protected final void   setValidRef(final HDLmModLines newValue) {
