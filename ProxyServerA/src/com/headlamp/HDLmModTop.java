...
/**
 * HDLmModTop short summary.
 *
 * HDLmModTop description.
 *
 * @version 1.0
 * @author Peter
 */
...
public class HDLmModTop extends HDLmMod {
  ...
  private HDLmModCompanies    companiesReference = null;
  private HDLmModReports      reportsReference = null;
  ...
  protected static HDLmPassThruStatus  checkWebSite(final String webSite, 
  	...
  	if (topPassThru == null)
		...
		/* Get the companies pass-through reference */
		HDLmModCompanies   companiesPassThru = topPassThru.getCompaniesReference();
		if (companiesPassThru == null)
		  HDLmAssertAction(false, "Companies pass-through reference is null");
  	/* Try to locate the web site in the companies tree map. If we 
  	   don't find the company entry, then the company entry must be
  	   created. */ 
  	HDLmModCompany   company = companiesPassThru.companiesTreeMap.get(webSite);
  	if (company == null) {
  		company = HDLmModCompany.addCompanyExtended(webSite);
			if (company == null)
				HDLmAssertAction(false, "New company instance not created");
  	}
  	/* Create an empty report instance. The lines of the report are 
  	   added below. */ 
  	HDLmModReport  report = HDLmModReport.buildNewReportExtended(company,
  	...
  	for (String line : lines) {
  		...
  		if (lineTokens.size() >= 16) {
  			lineCounter++;
  			String  lineName = HDLmModLine.buildLineName(lineCounter);
  			HDLmModLine  reportLine = HDLmModLine.buildLine(lineName,
  					                                            lineTokens);
  	...
  	if (ignoreListsCount == 0 &&
  			ignoreListsCount > 0) {
  		HDLmModList  dummyIgnoreList = HDLmModList.buildDummyIgnoreList();  	
  	...
  	if (ignoreListsCount > 0) {
  		HDLmModList  firstIgnoreList = company.getFirstIgnoreList();
  		if (firstIgnoreList == null)
  			HDLmAssertAction(false, "First ignore-list reference is null");
  		boolean   reportRv = HDLmModReport.checkReport(report, firstIgnoreList);
  ...
  /* This routine returns a reference to the companies instance that
     ...
     a null value. */
  protected HDLmModCompanies  getCompaniesReference() {
  ...
  /* This routine gets the company reference for a website. The company 
	...
	protected static HDLmModCompany  getOrAddWebSite(final String webSite) { 
		/* Check if the web site string reference (company name) passed 
		   by the caller is null */
		if (webSite == null) {
  ...
  /* The routine get the top pass-through tree node. The top 
    ...
    if (report == null)
  	...
  	if (executeMessage != null) {
			HDLmModLine.addErrorLine(report, 
					                          lineCounter, 
					                          "Execute", 
					                          "EN", 
					                          "None", 
  	...
  	if (stdOut != null) { 
	    HDLmModLine.addErrorLines(report, 
	    		                           lineCounter,
						                         "Stdout", 
						                         "EN", 
						                         "None", 
	...
	protected final void   setCompaniesReference(final HDLmModCompanies newValue) {
  ...
  protected final void   setLastModified(final Instant newLastModified) {
	...
	/* This routine sets the reports reference for a top instance 
     using a value passed by the caller */ 
  protected final void   setReportsReference(final HDLmModReports newValue) {
