...
/**
 * HDLmModDivision short summary.
 *
 * HDLmModDivision description.
 *
 * @version 1.0
 * @author Peter
 */
...
public class HDLmModDivision extends HDLmMod {
  ...
  protected HDLmModDivision(final String divisionName) { 
		/* Invoke the default constructor for the parent class. This is 
	     required by the Java language. */ 
	  super();
		/* Check if the division name passed by the caller, is null */
		if (divisionName == null) {
			String   errorText = "Division name string reference passed to the HDLmModDivision constructor is null";
	...
	protected HDLmModDivision(final JsonElement jsonElement) {
	  ...
	  MutableInt   errorCounter = new MutableInt(0);
		...
		if (errorMessages == null) {
			String  errorText = "Error message ArrayList allocation in HDLmModDivision constructor is null";
			throw new NullPointerException(errorText);
		}	
	  ...
	  countSites = curInteger;
		/* Mark the current division definition object as not executable, if  
		   the error count was greater than zero. This is actually done by 
		   setting the use mode field to a special value. */
  ...
  /* This method adds a site to the existing set of sites
	...
	protected static HDLmModDivision  buildDefaultDivision() {
		/* Declare and define a few variables */
		...
		HDLmModDivision newDivision = new HDLmModDivision(divisionName);
		if (newDivision == null) {
	  ...
	  /* Set the current status of the HDLmMod instance 
	     to "on". This avoids an error message later. */	  
	  newDivision.setUseMode("on");	
	...
	protected static JsonObject  buildDefaultDivisionJsonObject() {
		...
		HDLmModDivision  newDivision = buildDefaultDivision();
		if (newDivision == null) {
			String errorText = "New division instance was not created";
			HDLmAssertAction(false, errorText);
		}
