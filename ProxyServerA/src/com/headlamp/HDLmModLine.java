...
/**
 * HDLmModLine short summary.
 *
 * HDLmModLine description.
 *
 * @version 1.0
 * @author Peter
 */
...
public class HDLmModLine extends HDLmMod {
  ...
  protected HDLmModLine(final String lineName) { 
		/* Invoke the default constructor for the parent class. This is 
	     required by the Java language. */ 
	  super();
	...
	protected HDLmModLine(final JsonElement jsonElement) {
	  ...
	  detailsThree = curString;
		/* Mark the current line (report line) definition object as not executable,
	...
	protected static void  addErrorLine(final HDLmModReport curReport,
	  ...
	  /* Increment the total number of lines created so far */
		...
		String  lineName = HDLmModLine.buildLineName(lineCounter.getValue());
		/* Build the error line mostly from values passed by the caller */
		HDLmModLine  errorLine;
		errorLine = HDLmModLine.buildErrorLine(lineName, 
																						newDescription, 
																						newLanguage, 
																						newTicketPackage, 
																						newTestResults);  
    ...
    for (String curLine : linesArray) { 
    	...
    	/* Build and add the new error line */	
			HDLmModLine.addErrorLine(curReport, 
					                      lineCounter, 
			                          newDescription, 
			                          newLanguage,
			                          newTicketPackage, 
	...
	protected static HDLmModLine  buildErrorLine(final String lineName,
	                                              final String newDescription,
	                                              final String newLanguage,
	                                              final String newTicketPackage,
	                                              final String newTestResults) {
	  ...
	  /* Try to create the new line instance. The fields are set below. */
		HDLmModLine  newLine = new HDLmModLine(lineName);
		if (newLine == null)
  ...
  /* This routine build a report line instance from a set of tokens.
	...
	protected static HDLmModLine  buildLine(final String lineName,
			                                     final ArrayList<HDLmToken> lineTokens) {
		/* Check if the line name string reference passed by the caller is null */ 
	  if (lineName == null) {
	  ...
	  HDLmModLine  reportLine = new HDLmModLine(lineName);
  	/* Set a few fields in the new report instance */
