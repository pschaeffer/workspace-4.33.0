...
/**
 * HDLmModCompany short summary.
 *
 * HDLmModCompany description.
 *
 * @version 1.0
 * @author Peter
 */
...
public class HDLmModCompany extends HDLmMod {
  ...
  protected HDLmModCompany() { 
	...
	protected HDLmModCompany(final String companyName) { 
		/* Invoke the default constructor for the parent class. This is 
	     required by the Java language. */ 
	  super();
		/* Check if the company name passed by the caller, is null */
		if (companyName == null) {
			String   errorText = "Company name string reference passed to the HDLmModCompany constructor is null";
	...
	protected HDLmModCompany(final JsonElement jsonElement) {
	  ...
	  if (errorMessages == null) {
	  	String  errorText = "Error message ArrayList allocation in HDLmModCompany constructor is null";
	  	throw new NullPointerException(errorText);
	  }	
	  ...
	  if (curBoolean != null) {
		...
		/* Mark the current company definition object as not executable, if the error count
		   was greater than zero. This is actually done by setting the the use mode field 
		   to a special value. */
  ...
  private HDLmModData         data = null;
  private HDLmModLists        ignoreLists = null;
  private HDLmModReports      reports = null;
  private HDLmModRules        rules = null;
  /* This routine builds a company instance and does all of the
     ...
     tree. */ 
  protected static HDLmModCompany  addCompanyExtended(final String companyName) {
  	...
  	HDLmModTop  topPassThru = HDLmMain.getPassThruTopReference();
  	if (topPassThru == null)
  	...
  	/* We can now build a tree node for the company and everything
		...
		HDLmTree  topNode = HDLmModTop.getTopTree();
  ...
  protected void         addReport(final HDLmModReport curReport) {
  	...
  	HDLmModReports  reportsRef = reports;
	  if (reportsRef == null) {
  ...
  /* This method adds a report to the existing set of reports (zero, 
    ...
    ArrayList<String>  nodePath = reportsNode.getNodePath();
	  HDLmTree  reportTree = HDLmModReport.buildTreeExtended(nodePath, 
                                                           curReport);
  ...
  protected void         buildCompanyExtended() {
       ...
       created so far. */
	  HDLmModData   newData = getDataRef();
	  /* Build the data instance if it does not already exist */
	  if (newData == null) {
	    newData = new HDLmModData();
	    if (newData == null)
    ...
    /* An ignore-lists instance is created, if no ignore-lists instance 
	  ...
	  if (newIgnoreLists == null) {
		...
		/* A reports instance is created, if no reports instance has been 
		   created so far. */
		HDLmModReports   newReports = getReportsRef();
		/* Build the reports instance if it does not already exist */
	  if (newReports == null) {
		  newReports = new HDLmModReports();
		  if (newReports == null)
		...
		/* A rules instance is created, if no rules instance has been 
	     created so far. */
	  HDLmModRules   newRules = getRulesRef();
	  /* Build the rules instance if it does not already exist */
    if (newRules == null) {
	    newRules = new HDLmModRules();
	    if (newRules == null)
  ...
  /* This routine builds a tree node for a company (just one company) 
	...
	protected static HDLmTree  buildTreeExtended(final ArrayList<String> oldNodePath, 
		...
		/* Build a new tree node with the correct information */
		HDLmTree  companyTree = HDLmModCompany.buildTree(oldNodePath, 
				                                                  newValue);				 
		if (companyTree == null) {
			String  errorText = "New company tree node was not created";
			HDLmAssertAction(false, errorText);
       ...
       A data instance is created, if no data instance has been 
       created so far. */
    HDLmModData  newData = newValue.getDataRef();
    /* Build the data instance if it does not already exist */
	  if (newData == null) {
	    newData = new HDLmModData();
	    if (newData == null)
	  ...
	  HDLmTree  dataTree = HDLmModData.buildTree(nodePath, 
	                                                  newData);	
	  ...
	  if (newIgnoreLists == null) {
		...
		/* Build a tree node for the reports associated with every company. 
		   A reports instance is created, if no reports instance has been 
		   created so far. */
		HDLmModReports   newReports = newValue.getReportsRef();
		/* Build the reports instance if it does not already exist */
	  if (newReports == null) {
		  newReports = new HDLmModReports();
		  if (newReports == null)
	  ...
	  /* Build a tree node for the reports associated with every company */
		HDLmTree  reportsTree = HDLmModReports.buildTree(nodePath, 
                                                          newReports);
		/* Build a tree node for the rules associated with every company. 
	     A rules instance is created, if no rules instance has been 
	     created so far. */
	  HDLmModRules   newRules = newValue.getRulesRef();
	  /* Build the rules instance if it does not already exist */
    if (newRules == null) {
	    newRules = new HDLmModRules();
	    if (newRules == null)
    ...
    /* Build a tree node for the rules associated with every company */
	  HDLmTree  rulesTree = HDLmModRules.buildTree(nodePath, 
                                                      newRules);		
  ...
  protected HDLmModData  getDataRef() {
  	return data;
  }
  ...
  protected HDLmModLists  getIgnoreListsRef() {
  	return ignoreLists;
  }
  ...
  protected HDLmModReports  getReportsRef() {
  	return reports;
  }
  ...
  protected HDLmModRules  getRulesRef() {
  	return rules;
  }
  ...
  protected final void   setData(final HDLmModData newValue) {
	...
	/* This routine sets the ignore-lists reference for a company 
	   instance using a value passed by the caller */ 
	protected final void   setIgnoreLists(final HDLmModLists newValue) {
  ...
  protected final void   setLastModified(final Instant newLastModified) {
	...
	/* This routine sets the reports reference for a company 
	   instance using a value passed by the caller */ 
	protected final void   setReports(final HDLmModReports newValue) {
	...
	/* This routine sets the rules reference for a company 
     instance using a value passed by the caller */ 
  protected final void   setRules(final HDLmModRules newValue) {
