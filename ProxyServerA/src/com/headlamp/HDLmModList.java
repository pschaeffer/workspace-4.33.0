...
/**
 * HDLmModList short summary.
 *
 * HDLmModList description.
 *
 * @version 1.0
 * @author Peter
 */
...
public class HDLmModList extends HDLmMod {
  ...
  protected HDLmModList() { 
  	...
  	countIgnores = 0;
  	ignoreEntries = new TreeMap<String, HDLmModIgnore>();
	...
	/* This is a copy constructor for this class. It doesn't do
	   anything. All fields of this class will be set to the default 
	   values specified. */
	protected HDLmModList(final HDLmModList oldList) { 
		/* Invoke the default constructor for the parent class. This is 
	     required by the Java language. */ 
	  super();
	...
	protected HDLmModList(JsonElement jsonElement) {
	  ...
	  MutableInt   errorCounter = new MutableInt(0);
		...
		if (errorMessages == null) {
			String  errorText = "Error message ArrayList allocation in HDLmModList constructor is null";
			throw new NullPointerException(errorText);
		}		  
	  ...
	  /* We don't really want to use the count of ignore-list entries we 
	     ...
	     current ignore-list. */ 
    countIgnores = 0; 
		/* Mark the current list (set of ignore-list entries) definition object
		   as not executable, if the error count was greater than zero. This is 
		   actually done by setting the use mode field to a special value. */
  ...
  protected void         addIgnoreEntry(final HDLmModIgnore ignoreEntry,
	...
	protected static HDLmModList  buildDummyIgnoreList() {
	 /* Build an ignore-list */
	 HDLmModList  ignoreList = new HDLmModList();
	 if (ignoreList == null)
	 	 HDLmAssertAction(false, "New ignore-list was not created");
	 /* Build a dummy ignore-list entry */
	 HDLmModIgnore  ignoreEntry = HDLmModIgnore.buildDummyIgnoreLine();
	...
	protected static HDLmTree  buildTreeExtended(ArrayList<String> oldNodePath, 
		...
		/* Build a new tree node with the correct information */
		HDLmTree  listTree = HDLmModList.buildTree(oldNodePath,
				                                            newValue);
		if (listTree == null) {
		  String  errorText = "New list tree node was not created";
		  HDLmAssertAction(false, errorText);
		...
		/* Add all of the individual ignores as children of the list (set of
		   ignores) tree */ 
		for (Map.Entry<String, HDLmModIgnore> entry : newValue.getIgnoreEntries().entrySet()) {
			HDLmModIgnore  currentIgnore = entry.getValue();
			/* Try to build a tree node for the current ignore-list entry */
			HDLmTree  currentIgnoreTree = HDLmModIgnore.buildTree(newNodePath,
	                                                               currentIgnore);
  ...
  /* The routine below checks if the results of a verification test 
	...
	protected boolean      checkReportLine(final HDLmModLine reportLine) {
		/* Declare and define a few fields */
		boolean   rv = false;
		/* Check each of the ignore lines against the current report line */
		for (Map.Entry<String, HDLmModIgnore> entry : ignoreEntries.entrySet()) {
			HDLmModIgnore  ignoreListEntry = entry.getValue();
  ...
  protected Instant      getCreated() {
	...
	protected TreeMap<String, HDLmModIgnore>  getIgnoreEntries() {
