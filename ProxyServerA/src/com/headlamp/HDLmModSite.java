...
/**
 * HDLmModSite short summary.
 *
 * HDLmModSite description.
 *
 * @version 1.0
 * @author Peter
 */
...
public class HDLmModSite extends HDLmMod {
  ...
  protected HDLmModSite(final String siteName) { 
		/* Invoke the default constructor for the parent class. This is 
	     required by the Java language. */ 
	  super();
		/* Check if the site name passed by the caller, is null */
		if (siteName == null) {
			String  errorText = "Site name string reference passed to the HDLmModSite constructor is null";
	...
	protected HDLmModSite(JsonElement jsonElement) {
	  ...
	  MutableInt   errorCounter = new MutableInt(0);
		...
		if (errorMessages == null) {
			String  errorText = "Error message ArrayList allocation in HDLmModSite constructor is null";
			throw new NullPointerException(errorText);
		}	
	  ...
	  associatedNodeType = response.getAssociatedNodeType();
		...
		String  currentUseMode = response.getUseMode();
		if (currentUseMode != null)
		  setUseMode(currentUseMode);
	  ...
	  countRulesOrValues = curInteger;
		/* Mark the current site definition object as not executable, if  
	...
	/* This routine builds a default site and returns the
	   site to the caller */ 
	protected static HDLmModSite buildDefaultSite() {
		/* Declare and define a few variables */
		...
		HDLmModSite  newSite = new HDLmModSite(siteName);
		if (newSite == null) {
